...
from app.bot.messages import CODE_PRIVACY_NOTE, REGISTRATION_WARNING, SESSION_CANCELLED
...
async def _complete_registration(
    ...
    *,
    ...
    async with lock:
        ...
        await delete_sensitive_message(message)
        await state.clear()
        await _send_success_message(message, account)
        asyncio.create_task(
            notify_super_admin_new_user(message.bot, config, account)
        )
...
async def _send_success_message(message: Message, account: Account) -> None:
...
async def _cancel_registration_flow(
    message: Message,
    state: FSMContext,
    *,
...
async def receive_verification_code(
    ...
    if text == CODE_CANCEL:
        await _cancel_registration_flow(message, state, config=config)
        return
...
async def cancel_password_step(message: Message, state: FSMContext, config: AppConfig) -> None:
    data = await state.get_data()
    session_path = data.get("session_path")
    if session_path:
        _cleanup_pending_session(session_path)
